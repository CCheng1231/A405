...
plot(1./rhoBC,pressLevs*1.e-2,'b');
title('volume - temperature plot')
hold off;

rhoCB=[rhoCA,flipdim(rhoAB,2)];
pressCB=[press900Vec*1.e-2,pressLevs*1.e-2];

btmA=quad(@interpBC,1/rhoBC(end),1/rhoBC(1));

topA=quad(@interpCB,1/rhoCB(1),1/rhoCB(end));

work_done=topA-btmA
    
    
    function out=interpBC(x)        
        out=interp1(1./rhoBC,pressLevs*1.e-2,x);
    end
